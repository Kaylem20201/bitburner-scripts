...
export async function main(ns) {
	...
	async function crackAndNukeServer(cracks, target, portNum) {
		for (let i = 0; i < portNum; i++) {
			...
			switch (crack) {
				...
				case "SQLInject.exe":
					...
					break;
				default:
					ns.tprint("ERROR : Failure in crawlNetwork.ns, crackAndNukeServer()");
					ns.exit();
			}
		}
	...
	while (true) {
		for (const server of servers) {
			if (!ns.hasRootAccess) {
				...
				//Nuke the server
				crackAndNukeServer(cracks, server, portsNeeded);
