...
import { BATCH_CALCULATIONS_PORT, WEAKEN_THREADS_PER_HACK_THREAD } from "constantDefinitions";
...
function calculateHWGWThreads(ns: NS, nsServer: Server, hackPercent: number = 1.0, cores: number = 1): HWGWThreads | undefined {
    const player = ns.getPlayer();
    const maxMoney = nsServer.moneyMax;
    if (maxMoney === undefined) return undefined;

    const hackThreads = Math.ceil(hackPercent / ns.formulas.hacking.hackPercent(nsServer, player));
    const weakenToCounterHackThreads = Math.ceil(WEAKEN_THREADS_PER_HACK_THREAD * hackThreads);

    const moneyPerHackThread = ns.formulas.hacking.hackPercent(nsServer, player) * maxMoney;
    ...
    const growThreads = Math.ceil(ns.formulas.hacking.growThreads(uncappedServer, player, maxMoney + moneyDrained, cores));
    const weakenToCounterGrowThreads = Math.ceil(WEAKEN_THREADS_PER_GROW_THREAD * growThreads);
