...
async function runBatchOnServer(ns: NS, target: string, threadsObject: HWGWThreads, timings : HWGWTimings, hostname?: string): Promise<void> {
    ...
    for (let i = 0; i < timings.startTimes.length; i++) {
        if (timings.startTimes[i].type === 'weaken1') ns.exec(WEAKEN_SCRIPT, hostname, threadsObject.weaken1Threads, target);
        if (timings.startTimes[i].type === 'weaken2') ns.exec(WEAKEN_SCRIPT, hostname, threadsObject.weaken2Threads, target);
