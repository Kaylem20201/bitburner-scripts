...
export async function main(ns: NS): Promise<void> {
    ...
    if (typeof ns.args[0] !== 'string')
        throw new Error("Incorrect 'target' parameter");
    if (typeof ns.args[1] !== 'string')
        throw new Error("Incorrect 'threadsObject' parameter");
    if (typeof ns.args[2] !== 'string')
        throw new Error("Incorrect 'timings' parameter");
    let hostname = 'home';
    if (ns.args.length >= 4) {
        if (typeof ns.args[3] !== 'string')
            throw new Error("Incorrect 'hostname' parameter");
        hostname = ns.args[3];
    }
...
async function runBatchOnServer(
    ...
    for (const time of timings.startTimes)
        ns.print(time.time + ',' + time.type);
    ns.print('Threads: ');
    ...
    for (let i = 0; i < timings.startTimes.length; i++) {
        if (timings.startTimes[i].type === 'weaken1')
            ns.exec(
                WEAKEN_SCRIPT,
                hostname,
                threadsObject.weakenCounteringHackThreads,
                target,
            );
        if (timings.startTimes[i].type === 'weaken2')
            ns.exec(
                WEAKEN_SCRIPT,
                hostname,
                threadsObject.weakenCounteringGrowThreads,
                target,
            );
        if (timings.startTimes[i].type === 'hack')
            ns.exec(HACK_SCRIPT, hostname, threadsObject.hackThreads, target);
        if (timings.startTimes[i].type === 'grow')
            ns.exec(GROW_SCRIPT, hostname, threadsObject.growThreads, target);
        if (i < timings.startTimes.length - 1)
            await ns.sleep(
                timings.startTimes[i + 1].time - timings.startTimes[i].time,
            );
    }
